...
def solution(s):
    ...
    for x in range(1, len(s) + 1):
        # making the equal parts array by splitting the string
        parts = [s[i:i+x] for i in range(0, len(s), x)]

        # next, we check if all of the parts are equal, if so, return the length of the parts[], which is how many pieces are cut
