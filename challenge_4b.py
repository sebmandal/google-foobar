...
def solution(entrances, exits, path):
    ...
    def bfs(residual_graph, source, destination):
        queue = [source]
        visited = set()
        parent.clear()
        ...
        while queue:
            ...
            for neighbor, capacity in enumerate(residual_graph[current_node]):
    ...
    total_capacity = sum(sum(row) for row in path)

    # Build residual graph
    residual_graph = [[0] * (total_nodes + 2) for _ in range(total_nodes + 2)]
    for i in range(total_nodes):
        for j in range(total_nodes):
            residual_graph[i][j] = path[i][j]

    for entrance in entrances:
        residual_graph[total_nodes][entrance] = total_capacity

    for exit in exits:
        residual_graph[exit][total_nodes + 1] = total_capacity

    max_flow = 0

    while bfs(residual_graph, total_nodes, total_nodes + 1):
        path_flow = float("inf")
        node = total_nodes + 1
